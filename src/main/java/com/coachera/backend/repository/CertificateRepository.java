...
public interface CertificateRepository extends JpaRepository<Certificate, Integer> {
    ...
    @Query("SELECT DISTINCT c FROM Certificate c LEFT JOIN FETCH c.students sc LEFT JOIN FETCH sc.student")
    List<Certificate> findAllWithStudents();
    
    // Find certificates by student ID (through join entity)
    @Query("SELECT c FROM Certificate c JOIN c.students sc WHERE sc.student.id = :studentId")
    ...
    @Query("SELECT c FROM Certificate c LEFT JOIN FETCH c.students sc LEFT JOIN FETCH sc.student WHERE c.id = :id")
